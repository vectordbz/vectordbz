...
export async function seedWeaviateDB() {
  ...
  for (const col of collections) {
    try {
      ...
      // Create schema - base config without vector settings
      const schemaConfig = {
        class: col.name,
        invertedIndexConfig: {
          bm25: { b: 0.75, k1: 1.2 },
      ...
      if (col.vectors) {
        // Multiple named vectors - use vectorConfig (no class-level vectorizer)
        ...
        // Single vector - use class-level vector config
      ...
      // Enable multi-tenancy if specified
      if (col.multiTenant) {
        schemaConfig.multiTenancyConfig = {
          enabled: true,
        };
      }
