...
describe('Weaviate Client Integration Tests', () => {
  ...
  describe('Named Vectors Support', () => {
    ...
    it('should create collection with multiple named vectors', async () => {
      namedVectorsCollectionName = toPascalCase(
        generateTestCollectionName('weaviate_named_vectors'),
      );
    ...
    it('should search using specific named vector and verify vectors returned', async () => {
      ...
      if (
        docTitleVector.vectorType !== 'dense' ||
        !('data' in docTitleVector.value) ||
        !Array.isArray(docTitleVector.value.data)
      ) {
      ...
      if (
        !docContentVector ||
        docContentVector.vectorType !== 'dense' ||
        !('data' in docContentVector.value) ||
        !Array.isArray(docContentVector.value.data)
      ) {
        throw new Error('Invalid content_vector format in sample document');
      }

      // Search using title_vector from the inserted document
      const titleResult = await client.search(
        namedVectorsCollectionName,
        {
          title_vector: {
            key: 'title_vector',
            vectorType: 'dense',
      ...
      console.log(
        `  - Using content_vector from document: ${insertedNamedVectorDoc.primary.value}`,
      );
    }, 30000);
