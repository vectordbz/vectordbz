...
export const chromaDBCreateCollectionSchema: DynamicFormSchema = {
    ...
    sections: [
        ...
        },
        {
            ...
            items: [
                { key: 'hnsw_M', label: 'M (Max Connections)', type: 'number', defaultValue: 32, min: 4, max: 128, description: 'Number of edges per node. Higher = better recall but slower build' },
                { key: 'hnsw_construction_ef', label: 'EF Construction', type: 'number', defaultValue: 128, min: 4, max: 512, description: 'Search width during index build. Higher = better quality but slower' },
                { key: 'hnsw_search_ef', label: 'EF Search', type: 'number', defaultValue: 64, min: 4, max: 512, description: 'Search width during query. Higher = better recall but slower' },
            ],
        },
    ],
    showSubmit: true,
...
export class ChromaDBClient implements VectorDBClient {
    ...
    async getCollections(): Promise<GetCollectionsResult> {
        try {
            ...
            for (const col of collections) {
                try {
                    const collectionName = typeof col === 'string'
                        ? col
                        : (col as any)?.name || String(col);
    ...
    async getCollectionSchema(collection: string): Promise<GetCollectionSchemaResult> {
        try {
            ...
            const sample = await col.get({ limit: 1, include: [IncludeEnum.Embeddings, IncludeEnum.Metadatas, IncludeEnum.Documents, IncludeEnum.Distances] });
    ...
    async getDocuments(collection: string, options?: GetDocumentsOptions): Promise<GetDocumentsResult> {
        try {
            ...
            const getParams = {
                ...
                ...(ids && { ids }),
                ...(where && { where }),
                include: [IncludeEnum.Embeddings, IncludeEnum.Metadatas, IncludeEnum.Documents, IncludeEnum.Distances],
            };
    ...
    private buildChromaQueryParams(filter?: FilterQuery): {
        ...
        const where = metadataConditions.length > 0
     ...
     */
    private extractIdsFromConditions(idConditions: Array<{ operator: string; value: string | number | boolean }>): string[] | undefined {
        ...
        const eqIds = idConditions
            .filter(c => c.operator === 'eq' && (typeof c.value === 'string' || typeof c.value === 'number'))
    ...
    async search(collection: string, vectors: Record<string, DocumentVector>, options?: SearchOptions): Promise<SearchResult> {
        ...
        try {
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                if (options?.scoreThreshold !== undefined && score !== undefined && score < options.scoreThreshold) {
                    continue;
                }
    ...
    async createCollection(config: Record<string, unknown>): Promise<CreateCollectionResult> {
        try {
            ...
            if (config.hnsw_construction_ef) metadata['hnsw:construction_ef'] = config.hnsw_construction_ef;
        ...
        try {
            ...
            if (payload) {
                Object.entries(payload).forEach(([key, value]) => {
                    if (key !== 'document' && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')) {
                        metadata[key] = value;
                    }
                });
            }
