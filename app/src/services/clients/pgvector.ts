...
export const PGVECTOR_INDEX_TYPES = [
  { label: 'HNSW', value: 'hnsw', description: 'Fast approximate search, best for large datasets' },
  {
    label: 'IVFFlat',
    value: 'ivfflat',
    description: 'Balanced performance, good for medium datasets',
  },
] as const;
      ...
      items: [
        {
          ...
          options: [
            {
              label: 'Single Vector Column',
              value: 'single',
              description: 'One vector column named "embedding" (most common)',
            },
            {
              label: 'Multiple Vector Columns',
              value: 'multiple',
              description: 'Multiple named vector columns per row',
            },
          ],
        },
        {
      ...
      items: [
        ...
        },
        {
          ...
          itemFields: [
            ...
            {
              key: 'length',
              label: 'Length (for VARCHAR)',
              type: 'number',
              min: 1,
              max: 65535,
              showWhen: { field: 'type', operator: 'equals', value: 'varchar' },
            },
      ...
      items: [
        {
          ...
          description:
            'Index algorithm for vector search. HNSW is faster but uses more memory. IVFFlat is more memory-efficient.',
        },
        {
          key: 'hnsw_m',
          ...
          max: 128,
          description:
            'Number of edges per node. Higher = better recall but slower build and more memory',
        ...
        },
        {
          ...
          max: 1000,
          description:
            'Number of clusters. Typically sqrt(total_rows). Higher = better recall but slower. Create index after inserting some data.',
 ...
 */
function rowToDocument(row: any, vectorColumns: string[], primaryKey = 'id'): Document {
...
export class PgVectorClient implements VectorDBClient {
    ...
    try {
      ...
      const collections: Collection[] = await Promise.all(
        result.rows.map(async (row: { name: string }) => {
          try {
            const countResult = await client.query(`SELECT COUNT(*) as count FROM ${row.name}`);
            return {
              name: row.name,
  ...
  async getCollectionSchema(collection: string): Promise<GetCollectionSchemaResult> {
    ...
    try {
      ...
      const hasSearchableTextFields =
        Object.values(fields).some((field) => field.searchable === true) ||
  ...
  async getDocuments(
    ...
    try {
      ...
      try {
        ...
        if (options?.sort && options.sort.length > 0) {
          const sortFields = options.sort.map((sort, idx) => {
            ...
            if (
              ...
              !vectorColumns.includes(fieldName)
            ) {
        ...
        const result = await client.query(query, [...filterValues, limit, offset]);
    ...
    try {
      ...
      `,
        ...
        const ftsCondition = options?.lexicalQuery?.trim()
          ? buildFTSCondition(schemaResult.schema!, paramIndex, {
              lexicalQuery: options.lexicalQuery.trim(),
            })
          : '';
        const effectiveWhere = filterSQL
          ? ftsCondition
            ? `${filterSQL} AND ${ftsCondition}`
            : filterSQL
          : ftsCondition
            ? `WHERE ${ftsCondition}`
            : '';
        ...
        const queryParams = [
          vectorStr,
          ...filterValues,
          ...(options?.lexicalQuery?.trim() ? [options.lexicalQuery.trim()] : []),
          limit,
        ];
  ...
  async deleteDocuments(
    ...
    try {
      ...
      const schemaResult = await this.getCollectionSchema(collection);
      const schemaFields =
        schemaResult.success && schemaResult.schema ? schemaResult.schema.fields || {} : {};
    ...
    try {
      ...
      if (vectorType === 'single') {
        ...
        if (namedVectors.length === 0) {
          return {
            success: false,
            error: 'At least one named vector column is required for multiple vector configuration',
          };
        }
      ...
      if (vectorType === 'single') {
        ...
        await this.createVectorIndex(client, tableName, 'embedding', distance, indexType, config);
      } else {
        const namedVectors =
          (config.namedVectors as Array<{ name: string; size: number; distance: string }>) || [];
        for (const vec of namedVectors) {
  ...
  private async createVectorIndex(
    ...
    if (indexType === 'hnsw') {
      ...
      const efConstruction =
        (config.hnsw_ef_construction as number) || PGVECTOR_DEFAULTS.hnswEfConstruction;
  ...
  async upsertDocument(
    ...
    try {
      ...
      try {
        ...
        if (
          document.primary &&
          document.primary.value !== null &&
          document.primary.value !== undefined
        ) {
