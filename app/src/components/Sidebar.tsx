...
interface SidebarProps {
  ...
  onDropCollection: (connectionId: string, collectionName: string) => Promise<{ success: boolean; error?: string }>;
  onTruncateCollection: (connectionId: string, collectionName: string) => Promise<{ success: boolean; deletedCount?: number; error?: string }>;
  onCreateCollection: (connectionId: string, config: Record<string, unknown>) => Promise<{ success: boolean; error?: string }>;
  collapsed: boolean;
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '8px 0' }}>
          {connections.map(conn => (
            <ConnectionItem
              ...
              onCreateCollection={() => setCreateCollectionModal({
                open: true,
...
interface ConnectionItemProps {
  ...
  onTruncateCollection: (collectionName: string) => Promise<{ success: boolean; deletedCount?: number; error?: string }>;
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{
            ...
            {connection.config.host}{connection.config.port ? `:${connection.config.port}` : ''} {connection.config.tenant && connection.config.database ? `@${connection.config.tenant}/${connection.config.database}` : `${connection.config.database ? ` (${connection.config.database})` : ''}`}
          </div>
        </div>
      ...
      {connection.isExpanded && !connection.isLoading && (
        <div style={{ marginTop: 4 }}>
          {connection.collections.length === 0 ? (
            ...
            connection.collections.sort((a, b) => a.name.localeCompare(b.name)).map(collection => {
              const isOpen = tabs.some(
                t => t.connectionId === connection.id && t.collection.name === collection.name
              );
              const isActive = tabs.find(t => t.id === activeTabId)?.collection.name === collection.name &&
                      ...
                      items: [
                        {
                          ...
                          onClick: (e) => {
                            ...
                            Modal.confirm({
                              ...
                              content: (
                                <div>
                                  <p>Are you sure you want to truncate the collection <strong>{collection.name}</strong>?</p>
                                  <p><Text type="secondary">Current items: {collection.count.toLocaleString()}</Text></p>
                                  <p style={{ color: '#ff7875', fontSize: 12 }}>⚠️ This action cannot be undone!</p>
                                </div>
                              ),
                              ...
                              async onOk() {
                                ...
                                if (result.success) {
                                  message.success(`Truncated ${result.deletedCount || 0} documents`);
                                } else {
                            ...
                            Modal.confirm({
                              ...
                              content: (
                                <div>
                                  ...
                                  <p><Text type="secondary">This will permanently delete:</Text></p>
