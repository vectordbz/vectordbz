...
import { Modal, Input, InputNumber, Button, Space, Typography, message, Tooltip, Tag } from 'antd';
import { InfoCircleOutlined, ThunderboltOutlined } from '@ant-design/icons';
import { useTranslation } from 'react-i18next';
import Editor from '@monaco-editor/react';
import { CollectionSchema, Document, DocumentVector, COLLECTION_DEFAULT_VECTOR } from '../types';
...
const UpsertDocumentModal: React.FC<UpsertDocumentModalProps> = ({
  ...
  const [documentPrimary, setDocumentPrimary] = useState<{
    name: string;
    ...
    schemaFields.forEach((field) => {
      switch (field.type) {
        ...
        case 'object':
          ...
          break;
        case 'date':
          defaultPayload[field.name] = new Date().toISOString();
          break;
        case 'string':
          defaultPayload[field.name] = '';
          break;
        case 'unknown':
        default: // Set unknown field types to null
          defaultPayload[field.name] = null;
          break;
      }
    });
  ...
  const generateRandomVector = (key: string) => {
    ...
    try {
      const vectorJson = generateRandomVectorUtil(
        field.vectorType,
        'size' in field ? field.size : undefined,
      );
  ...
  const handleSubmit = async () => {
      ...
      try {
        ...
        if (field.vectorType === 'dense') {
          ...
          if (
            ...
            parsed.some((x) => typeof x !== 'number' || x < 0 || x > 255)
          ) {
          ...
          if (
            parsed.indices.some((x: any) => typeof x !== 'number') ||
            parsed.values.some((x: any) => typeof x !== 'number')
          ) {
    ...
    if (isEditing && editDocument?.primary) {
      ...
      !isAutoID &&
      documentPrimary?.value != null &&
      documentPrimary.value !== '' &&
      documentPrimary.value !== undefined
    ) {
      ...
      doc.primary = {
        ...
        value:
          primaryKey.type === 'number'
            ? typeof documentPrimary.value === 'number'
              ? documentPrimary.value
              : Number(documentPrimary.value)
    ...
    <Modal
      title={
        <Text strong style={{ fontSize: 16 }}>
          {isEditing ? t('upsert.editDocument') : t('upsert.createDocument')}
        </Text>
      }
      open={open}
      onCancel={onClose}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Space>
            ...
            <Button type="primary" loading={loading} onClick={handleSubmit}>
              {isEditing ? t('upsert.update') : t('upsert.create')}
            </Button>
          </Space>
        </div>
      }
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {primaryKey.type === 'number' ? (
                <div style={{ position: 'relative', width: '100%' }}>
                  <InputNumber
                    ...
                    onChange={(value) =>
                      setDocumentPrimary({
                        name: collectionSchema.primary.name,
                        value: value ?? null,
                      })
                    }
                  ...
                  >
                    <Tooltip
                      title={isAutoID ? t('upsert.autoGeneratedByDb') : t('upsert.autoGeneratedWhenEmpty')}
                    >
                      <InfoCircleOutlined
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ flex: 1 }} />
                    <Tooltip title={t('upsert.generateRandomVector')}>
                      <Button
                        type="text"
                        size="small"
                  ...
                  <TextArea
                    ...
                    onChange={(e) => setVectors((v) => ({ ...v, [field.name]: e.target.value }))}
            ...
            <div>
              ...
              <div
                style={{ marginTop: 8, border: '1px solid var(--border-color)', borderRadius: 6 }}
              >
                <Editor
                  height="260px"
                  language="json"
