...
ipcMain.handle(
  'db:getCollectionSchema',
  async (_event, connectionId: string, collection: string) => {
    const connection = clients.get(connectionId);
    if (!connection) {
      return { success: false, error: 'No active connection' };
    }
...
ipcMain.handle(
  ...
  async (_event, connectionId: string, collection: string, schema?: CollectionSchema | null) => {
    const connection = clients.get(connectionId);
    if (!connection) {
      return { success: false, error: 'No active connection' };
    }
    try {
      const capabilities = await connection.client.getSearchCapabilities(
        collection,
        schema ?? undefined,
      );
...
ipcMain.handle(
  ...
  async (
    ...
    return connection.client.deleteDocument(
      params.collection,
      params.primary,
...
ipcMain.handle(
  ...
  async (
    ...
    return connection.client.deleteDocuments(
      params.collection,
      params.filter,
      params.dataRequirements,
    );
...
ipcMain.handle(
  'db:createCollection',
  async (_event, connectionId: string, config: Record<string, unknown>) => {
    const connection = clients.get(connectionId);
    if (!connection) {
      return { success: false, error: 'No active connection' };
    }
...
ipcMain.handle(
  ...
  async (
    _event,
    connectionId: string,
    params: { collection: string; document: any; dataRequirements?: Record<string, string> },
  ) => {
    const connection = clients.get(connectionId);
    if (!connection) {
      return { success: false, error: 'No active connection' };
    }
    return connection.client.upsertDocument(
      params.collection,
    ...
    try {
      ...
      `);
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : 'Unknown error executing embedding function',
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Unknown error setting up embedding execution',
      };
    }
  },
);
